...
public class DaoUtil {
    ...
    public static StringBuffer fillParameters(final StringBuffer stringBuffer, final String[] propertyNames) {
        ...
        for (int i = 0; i < propertyNames.length; i++) {
            //第一个条件
            if (i == 0)
                stringBuffer.append("obj.").append(propertyNames[i]).append("=:").append(propertyNames[i]).append(" ");
            //第二个开始的条件
            if (i != 0)
                stringBuffer.append("and obj.").append(propertyNames[i]).append("=:").append(propertyNames[i]).append(" ");
        }
        //生成查询语句
        logger.info("组装的HQL：" + stringBuffer);
        return stringBuffer;
    }
    ...
    public static List<List> assemblingList(final Integer size, final Object... obj) throws Exception {
        ...
        if (length % size != 0) {
            ...
            for (int i = 0; i < outerLoop; i++) {
                ...
                innerList.addAll(Arrays.asList(obj).subList(i, i + size));
